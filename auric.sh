...
#-----------------------------------------------------------------------------------
#
# AURIC is mostly just vam with a pretty face, better error trapping, 
# pacman support for dependency installation, and a few additional features
#
# The name AURIC is a play on two words: AUR and Rick. It also means gold.
#-----------------------------------------------------------------------------------
# Author    :   Rick Ellis      https://github.com/rickellis/AURIC
...
download() {
    ...
    for ARG in $@; do
        ...
        if [[ $RESULTS == "[]" ]]; then
            echo -e "${red}not aur:${reset} ${ARG}. trying pacman:"
            sudo pacman -S --noconfirm --needed ${ARG}
        else
    
            # If a folder with the package name exists we exit
            if [ -d "$ARG" ]; then
                echo -e "${red}pkgskip:${reset} $ARG already exists in repo"
                continue
            fi
    ...
    if [ -z $1 ]; then
        ...
        for DIR in ./*; do
            ...
            if [[ $(git pull 2> /dev/null) != "Already up to date." ]]; then
                echo -e "${red}PROBLEM:${reset} ${DIR:2} needs to be reinstalled"
            else
                echo -e "${green}SUCCESS:${reset} ${DIR:2} is up to date"
            fi
            cd ..
        done
    else
        ...
        if [[ $(git pull 2> /dev/null) != "Already up to date." ]]; then
            echo -e "${red}PROBLEM:${reset} ${PKG} needs to be reinstalled"
        else
            echo -e "${green}SUCCESS:${reset} ${PKG} is up to date"
        fi
        cd ..
    fi
}
